...
export default function App() {
  ...
  // ── persist
  // cards now loaded from Supabase — no localStorage write needed
  useEffect(()=>{
    // encrypt text blocks before saving
    const safe = posts.map(p=>({...p,blocks:p.blocks?.map(b=>b.type==="text"?{...b,content:enc(b.content)}:b)||[]}));
    ss("posts_v4",safe);
  },[posts]);
  // reading state now stored in Supabase — no localStorage write needed
  // xp and milestones now stored in Supabase — no localStorage write needed
  ...
  useEffect(()=>{
    ...
    if(!el) return;
    const fn=()=>{ const pct=Math.min(100,Math.round(((el.scrollTop+el.clientHeight)/el.scrollHeight)*100)); setReadProgress(prev=>{ const next={...prev,[selectedPost.id]:Math.max(prev[selectedPost.id]||0,pct)}; saveReadingState({readProgressValue:next,blogStatsValue:blogStats,articlesReadValue:articlesRead}); return next; }); };
  ...
  useEffect(()=>{
    MILESTONES.forEach(m=>{
      if(totalLearned>=m.count && !earnedMilestones.includes(m.id)){
        const newMilestones = [...earnedMilestones, m.id];
        setEarnedMilestones(newMilestones);
        setXp(prev => {
          const newXp = prev + m.xp;
          saveUserProfile(newXp, newMilestones);
          return newXp;
        });
        setMilestonePopup(m);
  ...
  const saveUserProfile = async (newXp, newMilestones) => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    await supabase.from("user_profiles").upsert([{
      user_id: user.id,
      xp: newXp,
      earned_milestones: newMilestones,
  ...
  const loadUserProfile = async () => {
    ...
    if (data) {
      setXp(data.xp ?? 0);
      setEarnedMilestones(data.earned_milestones ?? []);
    }
  };
  ...
  const addXP = useCallback(async (amount) => {
    setXp(prev => {
      ...
      setEarnedMilestones(ms => {
        saveUserProfile(newXp, ms);
        return ms;
      });
      return newXp;
    });
  ...
  // ── reading state (Supabase)
  const loadReadingState = async () => {
    const { data: { user }, error } = await supabase.auth.getUser();
    if (error) { console.error("Get user error:", error); return; }
    if (!user) {
      setReadProgress({});
      setBlogStats({});
      setArticlesRead(0);
    ...
    const { data, error: loadError } = await supabase
      ...
      .eq("user_id", user.id)
      .maybeSingle();
    if (loadError) { console.error("Load reading state error:", loadError); return; }
    setReadProgress(data?.read_progress || {});
    setBlogStats(data?.blog_stats || {});
    setArticlesRead(data?.articles_read || 0);
  };

  const saveReadingState = async ({ readProgressValue, blogStatsValue, articlesReadValue }) => {
    const { data: { user }, error } = await supabase.auth.getUser();
    ...
    const { error: saveError } = await supabase
      ...
      .upsert([{
        user_id: user.id,
        read_progress: readProgressValue,
        blog_stats: blogStatsValue,
        articles_read: articlesReadValue,
        updated_at: new Date().toISOString(),
      }], { onConflict: "user_id" });
    if (saveError) console.error("Save reading state error:", saveError);
  };
  ...
  const loadCardsFromSupabase = async () => {
    ...
    if (user) {
      ...
      if (progressError) {
        console.error("Load progress error:", progressError);
        return;
      }
    ...
    const progressMap = new Map(
      progressRows.map((p) => [p.card_id, p])
    );

    const normalized = (cardsData || []).map((card) => {
      const progress = progressMap.get(card.id);

      return {
        ...card,
        synonyms: card.synonyms
          ? String(card.synonyms).split(",").map((s) => s.trim()).filter(Boolean)
          : [],
        interval: progress?.interval_days ?? 1,
        repetitions: progress?.repetitions ?? 0,
        easeFactor: Number(progress?.ease_factor ?? 2.5),
        due: progress?.due_at ? new Date(progress.due_at).getTime() : Date.now(),
      };
    });
  ...
  const generateCode = async (months) => {
    const { data, error } = await supabase.functions.invoke("admin-users", {
      body: { action: "generate-code", months }
    });
    if (error || !data?.code) { flash("❌ Код үүсгэж чадсангүй"); console.error(error); return; }
  ...
  const activateCode = async () => {
    ...
    if (!code) { setCodeError("Код оруулна уу."); return; }
    ...
    if (error || !data?.ok) {
      setCodeError(data?.error || "Код идэвхжүүлж чадсангүй.");
      return;
    }

    // update local user state with new premium status
    const { premiumUntil, months } = data;
    setUser(x => ({ ...x, user_metadata: { ...x.user_metadata, premium: true, premiumUntil } }));
    setShowPaywall(false);
    setAccessCode("");
    setToast(`✅ Premium идэвхжлээ! ${months} сар.`);
  };
  ...
  const testAdminFunction = async () => {
    ...
    if (error) {
      ...
      alert("Function worked. Open console.");
    }
  };
  const saveCard= async ()=>{
    ...
    if(editingCard){
      const { error } = await supabase
        .from("cards")
        .update({
          german: cardForm.german,
          mongolian: cardForm.mongolian,
          ipa: cardForm.ipa || null,
          example: cardForm.example || null,
          hint: cardForm.hint || null,
          synonyms: syns.join(", "),
      ...
      if (error) {
        console.error(error);
        flash("❌ Засах амжилтгүй");
        return;
      }
    } else {
      const slug = cardForm.german
        .toLowerCase()
        .trim()
        .replace(/\s+/g, "-")
        .replace(/ä/g, "ae")
      ...
      if (error) {
        console.error(error);
        flash("❌ Supabase хадгалалт амжилтгүй");
        return;
      }
    }

    await loadCardsFromSupabase();
    flash(editingCard ? "✅ Карт засагдлаа!" : "✅ Supabase-д хадгалагдлаа!");
  ...
  const saveProgressToSupabase = async ({
    userId, cardId, repetitions, easeFactor, intervalDays, dueAt,
  }) => {
    const { error } = await supabase
      .from("user_card_progress")
      .upsert(
  ...
  const rateCard = async (q) => {
    ...
    await saveProgressToSupabase({
      ...
      easeFactor: updated.easeFactor ?? 2.5,
      intervalDays: updated.interval ?? 1,
      dueAt: new Date(updated.due ?? Date.now()).toISOString(),
    });
  ...
  const openPost=post=>{
    ...
    if(!(readProgress[post.id]>0)) {
      const newArticlesRead = articlesRead + 1;
      setArticlesRead(newArticlesRead);
      saveReadingState({ readProgressValue: readProgress, blogStatsValue: blogStats, articlesReadValue: newArticlesRead });
    }
    setSelectedPost(post);
  };
  const finishReading=postId=>{
    const elapsed=(Date.now()-(readStartRef.current||Date.now()))/1000;
    const nextReadProgress={...readProgress,[postId]:100};
    setReadProgress(nextReadProgress);
    addXP(20);
    setBlogStats(prev=>{
      const totalRead=Object.keys(nextReadProgress).filter(k=>(nextReadProgress[k]||0)>=50).length;
      const fullyComplete=Object.values(nextReadProgress).filter(v=>v>=100).length;
      ...
      saveReadingState({ readProgressValue: nextReadProgress, blogStatsValue: next, articlesReadValue: articlesRead });
      return next;
    });
  };
            ...
            <div style={{ padding:20 }}>
                ...
                <div>
                  <div style={{ ...S.surface,marginBottom:20 }}>
                    ...
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
                      ...
                      <div>{S.lbl("🔀 Төстэй үгс",false)}<input value={cardForm.synonyms} onChange={e=>setCardForm(p=>({...p,synonyms:e.target.value}))} placeholder="die Empfindung, ..." style={S.inp()} /></div>
                    </div>
                      ...
                      <div key={c.id} style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"flex-start",gap:10 }}>
                        ...
                        <div style={{ display:"flex",gap:6 }}>
                          ...
                          <button onClick={async ()=>{
                            ...
                            if(error){ console.error(error); flash("❌ Устгах үед алдаа гарлаа."); return; }
                            await loadCardsFromSupabase();
                            flash("🗑️ Устгагдлаа.");
                          }} style={{ ...S.btn(C.red),padding:"6px 12px",fontSize:12 }}>🗑️</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {adminTab==="payment"&&(
                <div>
                  <div style={{ ...S.surface,marginBottom:16 }}>
                    <h3 style={{ margin:"0 0 14px",fontSize:15 }}>🔑 Premium код үүсгэх</h3>
                    <p style={{ fontSize:12,color:C.muted,margin:"0 0 12px" }}>Код үүсгэн хэрэглэгчид илгээнэ үү. Тэд кодоо оруулж premium идэвхжүүлнэ.</p>
                    <div style={{ display:"flex",gap:8,marginBottom:12 }}>
                      {[[3,"3 сар"],[6,"6 сар"],[12,"1 жил"]].map(([mo,label])=>(
                        <button key={mo} style={{ ...S.btn(C.accent),padding:"8px 14px",fontSize:13 }} onClick={()=>generateCode(mo)}>
                          + {label}
                        </button>
                      ))}
                    </div>
                    {newCode&&(
                      <div style={{ padding:"14px 16px",background:C.green+"15",border:`1px solid ${C.green}40`,borderRadius:10 }}>
                        <div style={{ fontSize:10,color:C.muted,marginBottom:4 }}>Хэрэглэгчид илгээх код:</div>
                        <div style={{ fontSize:24,fontWeight:800,color:C.green,letterSpacing:"3px" }}>{newCode}</div>
                      </div>
                    )}
                  </div>
                  <div style={S.surface}>
              ...
              {adminTab==="settings"&&(
                <div>
                  ...
                  <div style={{ display:"flex",gap:8,marginBottom:12 }}>
                    <button style={{ ...S.btn(C.accent),fontSize:12,padding:"8px 14px" }} onClick={testAdminFunction}>
                      🧪 Test admin function
                    </button>
                  </div>
                  <div style={{ padding:"12px 14px",background:C.red+"10",border:`1px solid ${C.red}30`,borderRadius:10 }}>
                    ...
                    <button style={{ ...S.btn(C.red),fontSize:12,padding:"8px 14px" }} onClick={()=>{if(window.confirm("Бүгдийг устгах уу?")){["posts_v4","u_db_v4"].forEach(k=>localStorage.removeItem(k));window.location.reload();}}}>Бүх өгөгдлийг устгах</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {showAddCard&&(
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }} onClick={e=>e.target===e.currentTarget&&setShowAddCard(false)}>
          <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:"26px 22px",width:"100%",maxWidth:520,maxHeight:"90vh",overflowY:"auto" }}>
            ...
            <div style={{ marginBottom:12 }}>{S.lbl("📝 Жишээ",false)}<textarea value={addForm.example} onChange={e=>setAddForm(p=>({...p,example:e.target.value}))} placeholder="Das ist eine Gurke.\nIch habe Gurken gekauft." style={{ ...S.inp(), minHeight:100, resize:"vertical", fontFamily:"inherit", paddingTop:12 }} /></div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
              ...
              <div>{S.lbl("🔀 Төстэй үгс",false)}<input value={addForm.synonyms} onChange={e=>setAddForm(p=>({...p,synonyms:e.target.value}))} placeholder="Frucht, Obst" style={S.inp()} /></div>
            </div>
            ...
            <div style={{ display:"flex",gap:8 }}>
              ...
              <button style={{ ...S.btn(C.green),flex:2 }} onClick={async ()=>{
                if(!addForm.german||!addForm.mongolian)return;
                const syns=addForm.synonyms?addForm.synonyms.split(",").map(s=>s.trim()).filter(Boolean):[];
                const slug=addForm.german.toLowerCase().trim().replace(/\s+/g,"-").replace(/ä/g,"ae").replace(/ö/g,"oe").replace(/ü/g,"ue").replace(/ß/g,"ss");
                const { error } = await supabase.from("cards").insert([{
                  slug,
                  german: addForm.german,
                  mongolian: addForm.mongolian,
                  ipa: addForm.ipa || null,
                  example: addForm.example || null,
                  hint: addForm.hint || null,
                  synonyms: syns.join(", "),
                ...
                if(error){ console.error(error); setToast("❌ Хадгалалт амжилтгүй"); return; }
                await loadCardsFromSupabase();
                setAddForm({german:"",mongolian:"",ipa:"",example:"",hint:"",synonyms:"",note:""});
                setShowAddCard(false);
                setToast("✅ Карт нэмэгдлээ!");
              }}>✅ Хадгалах</button>
            </div>
          </div>
        </div>
