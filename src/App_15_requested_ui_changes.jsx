...
export default function App() {
  ...
  const saveCard= async ()=>{
    ...
    if(editingCard){
      ...
      if (error) {
        console.error(error);
        flash(`❌ ${error.message || "Supabase хадгалалт амжилтгүй"}`);
        return;
      }
    }
      ...
      {showAddCard&&(
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }} onClick={e=>e.target===e.currentTarget&&setShowAddCard(false)}>
          <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:"26px 22px",width:"100%",maxWidth:520,maxHeight:"90vh",overflowY:"auto" }}>
            ...
            <div style={{ marginBottom:12 }}>{S.lbl("📝 Жишээ",false)}<textarea value={addForm.example} onChange={e=>setAddForm(p=>({...p,example:e.target.value}))} placeholder={"Das ist ein Apfel.\nNoch ein Beispiel..."} rows={4} style={{ ...S.inp(),resize:"vertical",lineHeight:1.6,whiteSpace:"pre-wrap" }} /></div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
            ...
            <div style={{ marginBottom:18 }}>{S.lbl("📌 Тэмдэглэл",false)}<textarea value={addForm.note} onChange={e=>setAddForm(p=>({...p,note:e.target.value}))} rows={2} style={{ ...S.inp(),resize:"vertical",lineHeight:1.6,whiteSpace:"pre-wrap" }} /></div>
            <div style={{ display:"flex",gap:8 }}>
              ...
              <button style={{ ...S.btn(C.green),flex:2 }} onClick={async ()=>{
                ...
                if(error){ console.error(error); setToast(`❌ ${error.message || "Хадгалалт амжилтгүй"}`); return; }
                await loadCardsFromSupabase();
