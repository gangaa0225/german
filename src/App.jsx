...
export default function App() {
  // ── auth
  const [user, setUser] = useState(() => ls("cu_v4",null));
  ...
  const NavBar=()=>(
    <nav style={{ display:"flex",background:C.surface,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:20,overflowX:"auto" }}>
      ...
      <div style={{ marginLeft:"auto",display:"flex",alignItems:"center",gap:8,padding:"0 12px" }}>
        ...
        <button onClick={()=>{ss("cu_v4",null);setUser(null);setView("home");}} style={{ background:"none",border:"none",color:C.dim,cursor:"pointer",fontSize:12 }}>Гарах</button>
      </div>
    </nav>
  );
          ...
          <>
            <div style={{ display:"flex",borderBottom:`1px solid ${C.border}`,background:C.surface,overflowX:"auto",flexShrink:0 }}>
              {[["cards","🃏 Карт"],["blog","📝 Блог"],["users","👥 Хэрэглэгч"],["payment","💳 Төлбөр"],["settings","🔧"]].map(([t,l])=>(
                <button key={t} onClick={()=>setAdminTab(t)} style={{ padding:"12px 15px",background:"none",border:"none",borderBottom:adminTab===t?`2px solid ${C.accent}`:"2px solid transparent",color:adminTab===t?C.accentL:C.muted,cursor:"pointer",fontSize:13,fontWeight:adminTab===t?600:400,whiteSpace:"nowrap" }}>{l}</button>
              ))}
            </div>
            ...
            <div style={{ padding:20 }}>
              ...
              {adminTab==="users"&&(
                <div>
                  <div style={{ ...S.surface,marginBottom:20 }}>
                    <h3 style={{ margin:"0 0 12px",fontSize:15 }}>🔑 Premium код үүсгэх</h3>
                    <button style={S.btn(C.accent)} onClick={()=>{const code="GERMAN-"+Math.random().toString(36).substring(2,8).toUpperCase();const codes=ls("premium_codes_v4",{});codes[code]={created:Date.now(),used:false};ss("premium_codes_v4",codes);setNewCode(code);flash("✅ Код үүслээ!");}}>+ Шинэ код</button>
                    {newCode&&<div style={{ marginTop:10,padding:"12px 16px",background:C.green+"15",border:`1px solid ${C.green}40`,borderRadius:10 }}><div style={{ fontSize:10,color:C.muted,marginBottom:3 }}>Хэрэглэгчид илгээх:</div><div style={{ fontSize:22,fontWeight:800,color:C.green,letterSpacing:"3px" }}>{newCode}</div></div>}
                  </div>
                  {Object.values(getUsers()).length===0?<p style={{ color:C.muted }}>Хэрэглэгч байхгүй.</p>:(
                    <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                      {Object.values(getUsers()).map(u=>{
                        const days=Math.floor((Date.now()-u.trialStart)/86400000),left=Math.max(0,TRIAL_DAYS-days);
                        return(
                          <div key={u.email} style={{ background:C.card,border:`1px solid ${u.premium?C.gold+"50":C.border}`,borderRadius:10,padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8 }}>
                            <div><div style={{ fontWeight:700 }}>{u.name}</div><div style={{ fontSize:12,color:C.muted }}>{u.email} · {days}өд</div></div>
                            <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                              {u.premium?<><span style={S.tag(C.gold)}>★ Premium</span><button onClick={()=>{const us=getUsers();us[u.email].premium=false;saveUsers(us);flash("Цуцлагдлаа.");}} style={{ ...S.btn(C.red),padding:"5px 10px",fontSize:11 }}>Цуцлах</button></>
                              :<><span style={S.tag(days>=TRIAL_DAYS?C.red:C.muted)}>{days>=TRIAL_DAYS?"Дууссан":`${left}өд`}</span><button onClick={()=>{const us=getUsers();us[u.email].premium=true;saveUsers(us);if(user?.email===u.email)setUser(x=>({...x,premium:true}));flash(`✅ ${u.email} premium!`);}} style={{ ...S.btn(C.green),padding:"5px 12px",fontSize:12 }}>Premium болгох</button></>}
                            </div>
                          </div>
                        );
                      })}
  ...
  if(view==="home") {
    ...
    return(
      <div style={S.wrap}><NavBar/><TrialBanner/><Toast/><MilestonePop/><PaywallModal/><AdminModal/>
        <div style={S.page}>
          {/* Hero greeting */}
          <div style={{ background:`linear-gradient(135deg,#1a1040,#0d1a35)`,border:`1px solid ${C.border}`,borderRadius:20,padding:"24px 24px 20px",marginBottom:20,position:"relative",overflow:"hidden" }}>
            ...
            <div style={{ fontSize:24,fontWeight:800,marginBottom:14 }}>{user.name} 👋</div>
  ...
  // ══════════════════════════════════════════════════════════
  if(view==="deck") return(
    <div style={S.wrap}><NavBar/><TrialBanner/><Toast/><PaywallModal/><AdminModal/>
      {showAddCard&&(
  ...
  if(view==="blog"&&selectedPost){
    ...
    return(
      <div style={{ ...S.wrap,display:"flex",flexDirection:"column" }}><NavBar/><TrialBanner/><MilestonePop/><PaywallModal/><AdminModal/>
